...
// mpic++ diy_broadcast_merge_serialized_03.cpp -I${DIY_INC} -lpthread
...
class BContainer{
  public:
    ...
    void GenerateData (vector<GPFitMsg>& gp_fitmsg_total)
    {
      FCCollection fcol;
      for(int i=0;i<5;i++){
        FCPoint fcp;
...
struct Block
{
  ...
  std::vector<GPFitMsg> gp_fitmsg_total;  
    ...
    bool verbose)                         //
{
  ...
  Generate_Data(b->gp_fitmsg_total);

  cout<<"vec size "<<b->gp_fitmsg_total.size()<<endl;
  cout<<"fcol size "<<b->gp_fitmsg_total.at(0).fcol.fPts.size()<<endl;
}
